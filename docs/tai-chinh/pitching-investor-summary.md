# DISO COMMERCE PLATFORM — Investor Pitch Summary

**Người trình bày:** CEO, Diso
**Đối tượng:** Nhà đầu tư
**Ngày:** 2026-03-26
**Phiên bản:** v1.0

---

## 1. CƠ HỘI THỊ TRƯỜNG

**Thị trường e-commerce Việt Nam đang bùng nổ, nhưng 920.000+ SMEs vẫn chưa có giải pháp bán hàng online bài bản.**

| Chỉ số | Con số |
|--------|--------|
| Thị trường TMĐT Việt Nam (2025) | **~28 tỷ USD**, tăng trưởng ~20%/năm |
| Số lượng SMEs | **920.000+** (98% tổng doanh nghiệp) |
| Affiliate marketing VN | **~700 triệu - 1 tỷ USD** |
| Social commerce (2025) | **5 tỷ USD**, dự kiến 10 tỷ USD vào 2030 |
| GMV tập trung Shopee + TikTok Shop | **97%** — DN phụ thuộc nặng vào sàn |

**Vấn đề:** Không có nền tảng nào tại Việt Nam cung cấp trọn bộ: Website + Affiliate + Reseller + Ambassador + Vận hành — trong 1 hệ thống duy nhất. Doanh nghiệp phải ghép 5-10 công cụ rời rạc, tốn kém và không kiểm soát được data.

---

## 2. GIẢI PHÁP: DISO COMMERCE PLATFORM

**1 nền tảng, 10 module, 4 tầng — từ bán hàng đến xây dựng đội ngũ nghìn người.**

### Kiến trúc nền tảng

```
TẦNG BÁN HÀNG        │ Website TMĐT + Bán hàng đa kênh (Shopee, Lazada, TikTok)
TẦNG VẬN HÀNH         │ Thanh toán + Vận chuyển + Quản lý đơn hàng, tồn kho, đối soát 4 loại
TẦNG TĂNG TRƯỞNG      │ Affiliate Marketing + Reseller Network + Đại sứ Thương hiệu  ← MOAT
TẦNG CHĂM SÓC         │ CRM, Loyalty, Analytics, Admin & Infrastructure
```

### Distribution Engine — Lõi cạnh tranh (MOAT)

> Website, thanh toán, vận chuyển — ai cũng làm được. Nhưng **hệ thống quản lý đội ngũ bán hàng bên ngoài** (Affiliate + Reseller + Ambassador) kết hợp **Đối soát 4 loại tự động + Chống gian lận nâng cao** — đây mới là rào cản cạnh tranh thực sự mà không đối thủ nào tại VN sở hữu.

```
┌─────────────────────────────────────────────────────────┐
│              DISTRIBUTION ENGINE (USP)                   │
│                                                          │
│  Affiliate ──┐                                           │
│              ├── Tracking ── Đối soát ── Chi trả tự động │
│  Reseller ───┤       │                                   │
│              │   Chống gian lận                           │
│  Ambassador ─┘   (click ảo, fake views, device fingerprint)│
│                                                          │
│  → Proven: 11+ brands (TPBank, HDBank, VNG, Yody...)     │
│  → 7 nền tảng social đã tích hợp                         │
│  → Network effect: Càng nhiều data → càng chính xác      │
└─────────────────────────────────────────────────────────┘
```

**Tại sao đây là moat:**
- **Network effect:** Càng nhiều brand → càng nhiều affiliate/reseller → càng nhiều brand
- **Data moat:** Dữ liệu chống gian lận, influencer scoring cải thiện theo thời gian — không thể mua hay copy
- **Switching cost:** Sau khi brand xây đội ngũ 100-1000 affiliate/reseller trên Diso, chuyển sang platform khác = mất toàn bộ network

### Lợi thế cạnh tranh khác

- **Lộ trình sở hữu 100%** — giai đoạn đầu dùng vendor, đạt ~10 KH thì mua đứt source → tự chủ hoàn toàn, không vendor lock-in
- **Modular** — khách hàng mua từng phần hoặc trọn bộ
- **Đặc thù VN** — COD (67% giao dịch), VietQR, Zalo ecosystem (70M+ users), MISA kế toán, E-Invoice
- **Social proof** — Đã triển khai module Ambassador cho 11+ thương hiệu (TPBank, HDBank, VNG, Yody...)

---

## 3. MÔ HÌNH KINH DOANH

### 3.1. Pricing Strategy — 4 gói sản phẩm

| | **AMBASSADOR ONLY** | **ECOMMERCE STARTER** | **ECOMMERCE GROWTH** | **ECOMMERCE ULTIMATE** |
|---|---|---|---|---|
| **Đối tượng** | Brand/Agency cần quản lý KOL/Affiliate | SME bắt đầu bán hàng online | DN đang tăng trưởng đa kênh | DN lớn, multi-brand |
| **Module** | Ambassador + Affiliate Tracking + Chống gian lận + Đối soát | Website + Thanh toán + Vận chuyển + Admin + **Affiliate Tracking** | Starter + OMS + Omnichannel + Affiliate/Reseller đầy đủ | Tất cả module + Ambassador + CRM/Loyalty + tùy chỉnh |
| **Giá/tháng** | **15.000.000 VNĐ** | **10.000.000 VNĐ** | **30.000.000 VNĐ** | **50.000.000 VNĐ** |
| **Setup fee** | 80.000.000 VNĐ | 50.000.000 VNĐ | 200.000.000 VNĐ | 400.000.000 VNĐ |
| **Phí giao dịch** | — | 0.5% GMV | 0.3% GMV | Thỏa thuận |

**Chiến lược gói:**

> **Ambassador Only** — Bán được NGAY từ tháng 1 (module đã proven). Nhắm vào brands/agency đang quản lý KOL bằng Excel. Không cần chờ build platform commerce.
>
> **Starter** — Gói nền tảng bắt buộc. Đã bao gồm **Affiliate Tracking cơ bản** (link tracking, dashboard, hoa hồng cơ bản) để khách hàng trải nghiệm sức mạnh Distribution Engine ngay từ đầu. Đây là bước đệm tự nhiên để nâng cấp lên Growth/Ultimate khi business phát triển.
>
> **Growth & Ultimate** — Upsell path rõ ràng: khi Starter có đơn hàng ổn định → cần OMS, đa kênh, affiliate/reseller đầy đủ → nâng cấp.

### 3.2. Chiến lược mở rộng thị trường — Vendor Partnership

Thay vì đầu tư build toàn bộ nền tảng ecommerce từ đầu, Diso sử dụng **Vendor Partnership** — hợp tác với nhà cung cấp công nghệ ecommerce có sẵn để ra thị trường nhanh, tiết kiệm chi phí và thời gian phát triển:

```
┌──────────────────────────────────────────────────────────────┐
│                  MÔ HÌNH VENDOR PARTNERSHIP                   │
│                                                               │
│  VENDOR (nhà cung cấp công nghệ)                              │
│  ├── Cung cấp nền tảng ecommerce có sẵn (SaaS/source code)   │
│  └── Diso trả phí sử dụng nền tảng                           │
│                                                               │
│  DISO (tự chủ sales & vận hành)                               │
│  ├── Tích hợp nền tảng vendor + module riêng của Diso         │
│  ├── Trực tiếp sales, onboarding & support khách hàng         │
│  ├── Toàn quyền kiểm soát relationship với khách              │
│  └── Khi đạt ~10 KH → mua đứt bộ source code từ vendor       │
│                                                               │
│  KHÁCH HÀNG                                                   │
│  ├── Giải pháp ecommerce trọn bộ từ Diso                     │
│  └── Support trực tiếp từ Diso (không qua trung gian)         │
│                                                               │
│  LỘ TRÌNH:                                                    │
│  Phase 1: Dùng nền tảng vendor → tập trung sales & thu KH    │
│  Phase 2: Đạt ~10 KH → mua đứt source code                   │
│  Phase 3: Tự chủ 100% công nghệ → margin cao hơn             │
└──────────────────────────────────────────────────────────────┘
```

**Lợi ích chiến lược:**
- **Giảm chi phí & thời gian phát triển:** Không cần build nền tảng ecommerce từ đầu, tập trung nguồn lực vào module lõi (Distribution Engine)
- **Ra thị trường nhanh:** Có sản phẩm bán ngay thay vì chờ 6-12 tháng phát triển
- **Diso tự chủ sales:** Toàn quyền kiểm soát khách hàng, pricing, và relationship — vendor chỉ cung cấp công nghệ
- **Lộ trình mua source:** Khi đạt ~10 khách hàng → mua đứt bộ source code → tự chủ 100% công nghệ, không còn phụ thuộc vendor

> **Ghi chú:** Giá trên là giá đề xuất ban đầu, có thể điều chỉnh theo phản hồi thị trường.

---

## 4. TỔNG VỐN ĐẦU TƯ & PHÂN BỔ

### Tổng chi phí phát triển sản phẩm

| Hạng mục | Chi phí (VNĐ) |
|----------|---------------|
| **10 module Core** | 6.200.000.000 |
| **Options ưu tiên cao** (Pancake, TikTok Shop, MISA, E-Invoice, Zalo OA) | 742.500.000 |
| **Vận hành & marketing (12 tháng)** | ~1.500.000.000 |
| **Dự phòng (15%)** | ~1.266.000.000 |
| **TỔNG VỐN CẦN HUY ĐỘNG** | **~9.700.000.000 (~9.7 tỷ VNĐ)** |

### Phân bổ vốn

```
┌──────────────────────────────────────────────────────┐
│                 PHÂN BỔ VỐN ~9.7 TỶ                  │
│                                                       │
│  ██████████████████████████  Phát triển sản phẩm 72% │
│  ██████████                  Vận hành & Marketing 15% │
│  ████████                    Dự phòng 13%             │
└──────────────────────────────────────────────────────┘
```

---

## 5. LỘ TRÌNH TUYỂN DỤNG & XÂY DỰNG ĐỘI NGŨ

### Phase 0: Founding Team (Tháng 0 — trước khi code)

| Vị trí | Số người | Ghi chú |
|--------|----------|---------|
| CEO (Founder) | 1 | Định hướng sản phẩm, gọi vốn, bán hàng |
| CTO / Tech Lead | 1 | Kiến trúc hệ thống, tech decisions |
| **Tổng** | **2 người** | |

> Chi phí: Founder không lương hoặc lương tối thiểu giai đoạn đầu.

### Phase 1: Core Team (Tháng 1-3) — Xây nền tảng

| Vị trí | Tuyển mới | Tổng | Rate/tháng | Chi phí 3 tháng |
|--------|-----------|------|------------|-----------------|
| PM | 1 | 1 | 60.000.000 | 180.000.000 |
| BE Senior | 2 | 2 | 40.000.000 | 240.000.000 |
| FE Senior | 1 | 1 | 30.000.000 | 90.000.000 |
| QC | 1 | 1 | 25.000.000 | 75.000.000 |
| **Tổng** | **+5** | **5 người** | **195 triệu/tháng** | **585.000.000** |

> *DevOps Phase 1 do 1 BE Senior kiêm nhiệm (setup CI/CD, infrastructure ban đầu). DevOps full-time tuyển từ Phase 2 khi hệ thống phức tạp hơn.*

**Sản phẩm đầu ra Phase 1:**
- Website TMĐT white-label LIVE
- Thanh toán VNPay + MoMo + COD hoạt động
- Vận chuyển GHN + GHTK + Viettel Post
- CI/CD & infrastructure nền tảng

### Phase 2: Mở rộng Team (Tháng 4-6) — Vận hành

| Vị trí | Tuyển mới | Tổng | Rate/tháng | Chi phí 3 tháng |
|--------|-----------|------|------------|-----------------|
| DevOps | +1 | 1 | 40.000.000 | 120.000.000 |
| BA | +1 | 1 | 25.000.000 | 75.000.000 |
| FE Junior | +1 | 2 FE | 20.000.000 | 60.000.000 |
| Design | +1 (part-time) | 1 | 15.000.000 | 45.000.000 |
| Team cũ | — | 5 | 195.000.000 | 585.000.000 |
| **Tổng** | **+4** | **9 người** | **295 triệu/tháng** | **885.000.000** |

**Sản phẩm đầu ra Phase 2:**
- OMS — quản lý đơn hàng tập trung, đối soát 4 loại
- Omnichannel — Shopee + Lazada đồng bộ
- Tích hợp Pancake + MISA kế toán
- **→ Sẵn sàng onboard khách hàng đầu tiên (gói Starter)**

### Phase 3: Growth Team (Tháng 7-9) — Tăng trưởng

| Vị trí | Tuyển mới | Tổng | Rate/tháng | Chi phí 3 tháng |
|--------|-----------|------|------------|-----------------|
| BE Junior | +1 | 3 BE | 25.000.000 | 75.000.000 |
| Sales/BD | +1 | 1 | 20.000.000 | 60.000.000 |
| CS (Customer Success) | +1 | 1 | 15.000.000 | 45.000.000 |
| Team cũ | — | 9 | 295.000.000 | 885.000.000 |
| **Tổng** | **+3** | **12 người** | **355 triệu/tháng** | **1.065.000.000** |

**Sản phẩm đầu ra Phase 3:**
- Affiliate Marketing system
- Reseller Network + Gamification
- TikTok Shop integration
- **→ Ra mắt gói Growth, bắt đầu có doanh thu định kỳ**

### Phase 4: Full Team (Tháng 10-12) — Brand & CRM

| Vị trí | Tuyển mới | Tổng | Rate/tháng | Chi phí 3 tháng |
|--------|-----------|------|------------|-----------------|
| Marketing | +1 | 1 | 20.000.000 | 60.000.000 |
| Sales/BD | +1 | 2 | 20.000.000 | 60.000.000 |
| Team cũ | — | 12 | 355.000.000 | 1.065.000.000 |
| **Tổng** | **+2** | **14 người** | **395 triệu/tháng** | **1.185.000.000** |

**Sản phẩm đầu ra Phase 4:**
- Ambassador program (đã proven với 11+ brands)
- CRM + Loyalty + Zalo OA
- E-Invoice tự động
- **→ Full platform, ra mắt gói Ultimate**

### Tổng quan đội ngũ theo thời gian

```
Tháng:    1───3     4───6     7───9     10──12    13──18
          │         │         │         │         │
Headcount:│  5      │  9      │  12     │  14     │  14-16
          │         │         │         │         │
Chi phí/  │ 195tr   │ 295tr   │ 355tr   │ 395tr   │ 395-430tr
tháng:    │         │         │         │         │

Tuyển:    PM        DevOps    BE Jr     Marketing  Tùy nhu cầu
          2 BE Sr   BA        Sales     Sales
          FE Sr     FE Jr     CS
          QC        Design(PT)
```

---

## 6. PHÂN TÍCH HOÀN VỐN (BREAK-EVEN ANALYSIS)

### 6.1. Giả định mô hình (Assumptions)

| Thông số | Giá trị | Ghi chú |
|----------|---------|---------|
| **Churn rate** | 5%/tháng (năm 1), 3%/tháng (năm 2+) | B2B SaaS VN trung bình 4-7%/tháng giai đoạn đầu |
| **CAC (Chi phí acquire 1 KH)** | ~30.000.000 VNĐ | Sales salary + marketing + demo + onboarding (~2 tháng effort) |
| **Ramp-up MRR** | KH mới đạt full MRR sau tháng thứ 2 | Tháng 1 = setup + onboarding, tháng 2 = active |
| **Mix gói mặc định** | 15% Amb Only, 35% Starter, 35% Growth, 15% Ultimate | Dựa trên phân bổ SME VN theo quy mô |
| **Phí GD thực tế** | 60-70% ước tính (không phải KH nào cũng đạt GMV trung bình) | Conservative |
| **Tăng giá** | Không tính (giữ giá cố định 24 tháng) | Upside tiềm năng |

### 6.2. Chi phí vận hành hàng tháng

**Giai đoạn build (Tháng 1-6):**

| Hạng mục | T1-3 | T4-6 |
|----------|------|------|
| Nhân sự | 195.000.000 | 295.000.000 |
| Cloud infrastructure | 10.000.000 | 15.000.000 |
| Tools & licenses | 5.000.000 | 10.000.000 |
| Office & misc | 10.000.000 | 10.000.000 |
| **Tổng/tháng** | **220.000.000** | **330.000.000** |

**Giai đoạn vận hành (Tháng 7+):**

| Hạng mục | T7-12 | T13+ |
|----------|-------|------|
| Nhân sự | 355.000.000 → 395.000.000 | 395.000.000 |
| Cloud infrastructure | 20.000.000 | 30.000.000 |
| Tools & licenses | 10.000.000 | 10.000.000 |
| Sales & marketing | 30.000.000 | 50.000.000 |
| Office & misc | 15.000.000 | 15.000.000 |
| **Tổng/tháng** | **~430.000.000 → 450.000.000** | **~500.000.000** |

### 6.3. Unit Economics — Doanh thu & Giá trị mỗi khách hàng

| Gói | Monthly fee | Setup fee | Phí GD (est.) | **MRR/KH** | **LTV 24 tháng** | **LTV/CAC** |
|-----|-------------|-----------|---------------|------------|-----------------|-------------|
| **Ambassador Only** | 15.000.000 | 80.000.000 | — | **15.000.000** | **410.000.000** | **13.7x** |
| **Ecommerce Starter** | 10.000.000 | 50.000.000 | ~1.750.000 | **11.750.000** | **332.000.000** | **11.1x** |
| **Ecommerce Growth** | 30.000.000 | 200.000.000 | ~5.250.000 | **35.250.000** | **1.046.000.000** | **34.9x** |
| **Ecommerce Ultimate** | 50.000.000 | 400.000.000 | ~10.500.000 | **60.500.000** | **1.852.000.000** | **61.7x** |

> **Công thức LTV:** Setup fee + (MRR × 24 tháng × retention rate)
> *Retention rate = (1 - churn)^24. Với churn 5%/th năm 1, 3%/th năm 2: effective retention 24 tháng ≈ ~46%*
> **Phí GD conservative:** Tính 70% GMV trung bình (Starter 350tr, Growth 1.75 tỷ, Ultimate 3.5 tỷ thực tế)
> **CAC:** ~30tr/KH (bao gồm sales effort, marketing, onboarding)
> **LTV/CAC > 3x** là benchmark tốt cho B2B SaaS → tất cả gói đều **rất tốt**

### 6.4. Kịch bản hoàn vốn — Có tính churn & CAC

#### Mục tiêu 1: Hòa vốn vận hành (Cover chi phí ~500 triệu/tháng sau tháng 13)

**Net MRR = Gross MRR - Churned MRR**

| Kịch bản | Tổng KH active | Amb. | Starter | Growth | Ultimate | Gross MRR | Churn MRR (3%) | **Net MRR** | Trạng thái |
|-----------|---------------|------|---------|--------|----------|-----------|----------------|-------------|------------|
| **A — Ambassador lead** | 22 | 8 | 6 | 5 | 3 | 508.750.000 | -15.262.500 | **493.487.500** | ✅ Hòa vốn |
| **B — Cân bằng** | 25 | 5 | 8 | 8 | 4 | 617.000.000 | -18.510.000 | **598.490.000** | ✅ Hòa vốn |
| **C — Growth/Ult. lead** | 20 | 3 | 4 | 8 | 5 | 629.750.000 | -18.892.500 | **610.857.500** | ✅ Hòa vốn |

> **→ Cần khoảng 20-25 khách hàng active (sau churn) để hòa vốn vận hành.**
> **→ Ambassador Only giúp có MRR sớm hơn 6-9 tháng so với chờ build full platform.**

#### Mục tiêu 2: Hoàn vốn đầu tư ban đầu (~9.7 tỷ)

**Công thức:**
```
Lũy kế tháng N = Σ (Net MRR + Setup fee KH mới - Chi phí vận hành - CAC × KH mới)

Trong đó:
- Net MRR      = Gross MRR tháng trước + MRR KH mới - MRR KH churn
- Setup fee    = Số KH mới × Avg setup fee (~130tr, theo mix gói)
- Chi phí VH   = Theo bảng 6.2
- CAC          = 30tr × số KH mới
```

| Kịch bản | KH mới/tháng | Churn/tháng | Thời gian hoàn vốn | KH active tại breakeven |
|-----------|-------------|-------------|---------------------|------------------------|
| **Bảo thủ** | 3-4 | 5% Y1, 3% Y2 | **Tháng 30-36** (2.5-3 năm) | ~50-65 KH active |
| **Trung bình** | 5-7 | 5% Y1, 3% Y2 | **Tháng 22-26** (~2 năm) | ~65-85 KH active |
| **Lạc quan** | 8-10 | 5% Y1, 3% Y2 | **Tháng 18-22** (~1.5 năm) | ~85-110 KH active |

> **→ Hoàn vốn trong 2-3 năm. Với 920.000+ SMEs tại VN, chỉ cần ~0.01% = 92 KH.**

### 6.5. Chi tiết kịch bản trung bình (Recommended)

**Giả định:** 5 KH mới/tháng, churn 5% Y1 → 3% Y2, mix gói mặc định, CAC 30tr/KH

```
         KH     KH      KH     GROSS    CHURN    NET      SETUP    CHI PHÍ    LỢI NHUẬN   LŨY KẾ
THÁNG    MỚI    CHURN   ACTIVE MRR(tr)  MRR(tr)  MRR(tr)  FEE(tr)  VH(tr)     RÒNG(tr)    (triệu)
──────   ────   ─────   ────── ───────  ───────  ───────  ───────  ───────    ─────────   ────────
T1-3     2(A)   0       2      30       0        30       160      220        -30/th      -660
T4-6     3(A+S) 0       7      115      -6       109      290      330        -50→+70/th  -1.530
T7-9     +4/th  -1/th   16     375      -19      356      520      430        -50→+50/th  -2.200
T10-12   +5/th  -1/th   28     640      -32      608      650      450        +150→+200   -1.600
T13-15   +5/th  -1/th   40     920      -28      892      650      500        +400/th     +0
T16-18   +6/th  -1/th   55     1.270    -38      1.232    780      500        +730→+800   +2.400
T19-22   +7/th  -2/th   73     1.700    -51      1.649    910      500        +1.100/th   +7.000
T23-24   +7/th  -2/th   83     1.950    -59      1.891    910      500        +1.400/th   +9.800
```

> *T1-6: chủ yếu bán Ambassador Only + Starter pilot (sản phẩm đã có/đang build)*
> *T7+: bắt đầu bán Growth khi OMS + Omnichannel + Affiliate hoàn thiện*
> *T10+: Ultimate khi full platform ready*

**Tóm tắt:**
- **Hòa vốn vận hành:** ~Tháng 13-15, tại ~35-40 KH active
- **Hoàn vốn đầu tư:** ~Tháng 23-24, tại ~80-85 KH active
- **ARR tại breakeven:** ~22-24 tỷ VNĐ/năm
- **Net Revenue Retention (NRR):** ~95-97% sau năm 2 (churn thấp + upsell Starter → Growth)

---

## 7. TẠI SAO ĐẦU TƯ BÂY GIỜ?

### Timing — Thị trường chín muồi

1. **920.000+ SMEs** cần giải pháp — không có nền tảng nào phục vụ trọn bộ tại VN
2. **97% GMV phụ thuộc sàn** — DN đang tìm cách thoát phụ thuộc Shopee/TikTok
3. **Affiliate marketing tăng 30x** trong 2 năm — nhưng không có công cụ quản lý bài bản
4. **Zalo ecosystem 70M+ users** — lợi thế mà không competitor quốc tế nào có

### Diso có gì sẵn?

| Tài sản | Chi tiết |
|---------|----------|
| **Module Ambassador đã proven** | Triển khai cho 11+ thương hiệu (ngân hàng, retail, gaming) |
| **7 nền tảng social** đã tích hợp | TikTok, YouTube, Facebook, Instagram, Threads, Shopee, Google |
| **Chống gian lận** built-in | Fake views, fake followers — đã xử lý hàng triệu data points |
| **Tech stack sẵn sàng** | Go + NestJS + React, kiến trúc microservice, đã production |
| **Team có kinh nghiệm** | Đã build và vận hành hệ thống cho TPBank, HDBank, VNG, Yody... |

### Moat — Distribution Engine (Chi tiết tại Mục 2)

- **Network effect:** Càng nhiều brand → càng nhiều affiliate/reseller → càng nhiều brand
- **Data moat:** Dữ liệu chống gian lận, influencer scoring cải thiện theo thời gian — không thể mua hay copy
- **Đặc thù VN:** COD flow, Zalo ecosystem, MISA kế toán, E-Invoice — competitor quốc tế không có
- **Switching cost:** Sau khi brand xây đội ngũ 100-1000 affiliate/reseller trên Diso → chuyển platform = mất toàn bộ network
- **Vendor Partnership:** Dùng nền tảng vendor có sẵn → ra thị trường nhanh, tập trung sales → đạt 10 KH thì mua đứt source → tự chủ 100%

---

## 8. TÓM TẮT CHO NHÀ ĐẦU TƯ

```
┌──────────────────────────────────────────────────────────────────┐
│                                                                  │
│   VỐN CẦN HUY ĐỘNG:     ~9.7 tỷ VNĐ                            │
│                                                                  │
│   SẢN PHẨM:             10 module, nền tảng e-commerce trọn bộ  │
│   MOAT:                 Distribution Engine (Affiliate +         │
│                          Reseller + Ambassador + Chống gian lận) │
│   THỊ TRƯỜNG:           920.000+ SMEs tại Việt Nam               │
│   MÔ HÌNH DOANH THU:   SaaS (MRR) + setup fee + phí GD         │
│                          + Vendor Partnership (dùng→mua source)  │
│                                                                  │
│   4 GÓI:  Ambassador Only 15tr | Starter 10tr | Growth 30tr     │
│            Ultimate 50tr  (setup 50-400tr)                          │
│   HÒA VỐN VẬN HÀNH:    ~35-40 KH active (tháng 13-15)           │
│   HOÀN VỐN ĐẦU TƯ:     ~80-85 KH active (tháng 23-24)          │
│   ARR MỤC TIÊU NĂM 2:  ~22-24 tỷ VNĐ                           │
│   LTV/CAC:              11x - 62x (tùy gói)                     │
│                                                                  │
│   ĐỘI NGŨ:             6 → 9 → 12 → 14 người (12 tháng)        │
│   DOANH THU SỚM:       Ambassador Only bán được từ tháng 1      │
│   FULL PLATFORM:        Tháng 12                                 │
│                                                                  │
│   LỢI THẾ SẴN CÓ:      Ambassador proven (11+ brands)           │
│                          Tech stack production-ready              │
│                          Vendor partnership (dùng→mua source)     │
│                                                                  │
│   ASK:  ~9.7 tỷ VNĐ cho 12-18 tháng phát triển + go-to-market  │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```

---

## 9. CHIẾN LƯỢC GO-TO-MARKET

> **⚠️ Cần xây dựng chi tiết**
>
> Nội dung cần bổ sung:
> - Chiến lược tiếp cận khách hàng đầu tiên (first 10, first 50, first 100)
> - Kênh acquisition: direct sales, partnership, content marketing, referral
> - Sales cycle & conversion funnel dự kiến
> - Chiến lược pricing/promotion giai đoạn đầu (early adopter incentive)
> - Phân bổ ngân sách marketing theo phase
> - KPIs theo từng giai đoạn (MQL, SQL, conversion rate, time-to-close)

---

## 10. ĐIỀU KHOẢN ĐẦU TƯ (DEAL STRUCTURE)

> **⚠️ Cần xây dựng chi tiết**
>
> Nội dung cần bổ sung:
> - Hình thức đầu tư (equity, convertible note, SAFE)
> - Định giá pre-money / post-money
> - Tỷ lệ cổ phần chào bán
> - Phân bổ vốn chi tiết (product, team, marketing, operations)
> - Milestone-based disbursement (nếu có)
> - Quyền lợi nhà đầu tư (board seat, anti-dilution, vesting)
> - Exit strategy & timeline dự kiến

---

## PHỤ LỤC A: CHI TIẾT MODULE THEO GÓI

### Gói AMBASSADOR ONLY

| Module | Chức năng chính |
|--------|----------------|
| M8. Ambassador | Campaign KOL/KOC, 7 nền tảng social, duyệt content, analytics |
| Affiliate Tracking | Link tracking, hoa hồng, dashboard cho affiliate |
| Chống gian lận | Fake views, fake followers, device fingerprint |
| Đối soát & chi trả | Tính thưởng tự động, lịch chi trả, export báo cáo |

> *Gói này bán được ngay từ tháng 1 — không cần chờ build commerce platform. Nhắm vào brands/agency đang quản lý KOL bằng Excel/Google Sheet.*

### Gói ECOMMERCE STARTER (nền tảng bắt buộc)

| Module | Chức năng chính |
|--------|----------------|
| M1. Website TMĐT | Quản lý SP, giỏ hàng, checkout, SEO, white-label |
| M3. Thanh toán | VNPay, MoMo, COD, VietQR, refund |
| M4. Vận chuyển | GHN, GHTK, Viettel Post, tracking |
| **Affiliate Tracking (basic)** | **Link tracking, dashboard affiliate, hoa hồng cơ bản** |
| M10. Admin (Basic) | Dashboard, phân quyền cơ bản, hosting |

> *Starter đã bao gồm Affiliate Tracking cơ bản — khách hàng trải nghiệm sức mạnh Distribution Engine ngay từ đầu. Đây là bước đệm tự nhiên để nâng cấp lên Growth/Ultimate. **Gói Starter là gói nền tảng bắt buộc để có thể nâng cấp lên các gói cao hơn.***

### Gói ECOMMERCE GROWTH (thêm 3-4 module)

| Module bổ sung | Chức năng chính |
|---------------|----------------|
| M5. OMS | Quản lý đơn tập trung, tồn kho, đối soát 4 loại, RBAC |
| M2. Omnichannel | Shopee + Lazada + Pancake sync |
| M6. Affiliate **HOẶC** M7. Reseller (đầy đủ) | Hoa hồng đa tier, gamification, chống gian lận nâng cao, chi trả tự động |
| Option: TikTok Shop / MISA | Tùy chọn |

### Gói ECOMMERCE ULTIMATE (full platform)

| Module bổ sung | Chức năng chính |
|---------------|----------------|
| M6 + M7 (cả hai) | Affiliate + Reseller full |
| M8. Ambassador | Campaign, 7 nền tảng, duyệt content, analytics |
| M9. CRM & Loyalty | Tích điểm, hạng thành viên, email/SMS, Zalo OA |
| M10. Full Admin | BI analytics, monitoring, disaster recovery |
| Options ưu tiên cao | E-Invoice, Zalo OA, MISA, Pancake, TikTok Shop |

---

## PHỤ LỤC B: SO SÁNH CẠNH TRANH

| | Haravan | Sapo | Shopify | **Diso** |
|---|---|---|---|---|
| **Giá** | 200K-3M/th | 170K-1M/th | $29-299/th | 10-50M/th |
| **Phân khúc** | SME nhỏ | SME nhỏ | SME-MidMarket | **SME-Enterprise** |
| **Affiliate** | Không | Không | App bên thứ 3 | **Built-in (mọi gói)** |
| **Reseller** | Không | Không | Không | **Built-in** |
| **Ambassador** | Không | Không | Không | **Built-in (proven 11+ brands)** |
| **Chống gian lận** | Không | Không | Cơ bản | **Nâng cao** |
| **Đối soát 4 loại** | Không | Không | Không | **Có** |
| **Zalo ecosystem** | Cơ bản | Cơ bản | Không | **Full** |
| **Sở hữu data** | Hạn chế | Hạn chế | Hạn chế | **100%** |
| **White-label** | Hạn chế | Hạn chế | Hạn chế | **Full** |
| **Vendor Partnership** | Không | Không | Không | **Có (dùng nền tảng vendor → mua source khi scale)** |

> **Diso không cạnh tranh trực tiếp với Haravan/Sapo ở phân khúc giá rẻ.** Diso nhắm vào DN cần Distribution Engine — xây đội ngũ bán hàng bên ngoài (affiliate, reseller, ambassador), sở hữu data, và kiểm soát brand. Đây là phân khúc không ai phục vụ tại VN.

---

*Diso Commerce Platform — Investor Pitch Summary v1.0*
*Ngày: 2026-03-26*
